...
Then /^I should see "(.*?)" in the list of users$/ do |content|
  expect( find('.found_users') ).to have_content content
end

Then /^I should(?: not|n't) see "(.*?)" in the list of users$/ do |content|
  expect( find('.found_users') ).to_not have_content content
end

Then /^the original user should be "(.*?)"$/ do |username|
  expect( find('#util-links #original-user') ).to have_content (username + " acting as")
end
